...
#include <stdint.h>
#define DT_DRV_COMPAT ti_tca9539

/**
 * @file Driver for TCA9539 and PCAL95XX I2C-based GPIO driver.
 */
...
LOG_MODULE_REGISTER(gpio_tca9539);
...
/** Configuration data */
struct gpio_tca9539_config {
	/* gpio_driver_config needs to be first */
	struct gpio_driver_config common;
	struct i2c_dt_spec bus;
	uint8_t capabilities;
#ifdef CONFIG_GPIO_TCA9539_INTERRUPT
	struct gpio_dt_spec int_gpio;
#endif
};

/** Runtime driver data */
struct gpio_tca9539_drv_data {
 ...
 *
 * @param dev Device struct of the TCA9539.
...
{
	const struct gpio_tca9539_config *const config = dev->config;
	uint16_t port_data, value;
	int ret;

	ret = i2c_burst_read_dt(&config->bus, reg, (uint8_t *)&port_data, sizeof(port_data));
	if (ret != 0) {
		LOG_ERR("TCA9539[0x%X]: error reading register 0x%X (%d)", config->bus.addr, reg,
			ret);
		return ret;
	}
	...
	*buf = value;
	...
	LOG_DBG("TCA9539[0x%X]: Read: REG[0x%X] = 0x%X, REG[0x%X] = 0x%X", config->bus.addr, reg,
			  ...
			  uint16_t value)
{
	...
	LOG_DBG("TCA9539[0x%X]: Write: REG[0x%X] = 0x%X", config->bus.addr, reg, buf[1]);

	ret = i2c_write_dt(&config->bus, buf, sizeof(buf));
	if (ret == 0) {
		*cache = value;
	} else {
		LOG_ERR("TCA9539[0x%X]: error writing to register 0x%X "
			"(%d)",
...
static int write_port_regs(const struct device *dev, uint8_t reg, uint16_t *cache, uint16_t value)
{
	...
	uint8_t buf[3];
	int ret;
	LOG_DBG("TCA9539[0x%X]: Write: REG[0x%X]=0x%02X, REG[0x%X]=0x%02X", config->bus.addr, reg,
		(value & 0xFF), (reg + 1), (value >> 8) & 0xFF);
	buf[0] = reg;
...
{
	struct gpio_tca9539_drv_data *const drv_data =
		(struct gpio_tca9539_drv_data *const)dev->data;
...
static inline int update_direction_regs(const struct device *dev, uint16_t value)
{
	...
	return write_port_regs(dev, REG_CONF_PORT0, &drv_data->reg_cache.dir, value);
}
...
#endif /* CONFIG_GPIO_TCA9539_INTERRUPT */
...
{
	return update_direction_regs(dev, 0x0000);
}
 ...
 * @return 0 if successful, failed otherwise
 */
static int gpio_tca9539_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	int ret;
	struct gpio_tca9539_drv_data *const drv_data =
		(struct gpio_tca9539_drv_data *const)dev->data;
	...
	if (ret) {
		LOG_ERR("TCA9539[0x%X]: error setting pin direction (%d)", config->bus.addr, ret);
		goto done;
	}

	ret = setup_pin_pullupdown(dev, pin, flags);
	if (ret) {
		LOG_ERR("TCA9539[0x%X]: error setting pin pull up/down "
			"(%d)",
			config->bus.addr, ret);
		goto done;
	}
	LOG_INF("Config pin %d: dir=0x%04X, output=0x%04X", pin, drv_data->reg_cache.dir,
		drv_data->reg_cache.output);
done:
	k_sem_give(&drv_data->lock);
	return ret;
}

static int gpio_tca9539_port_get_raw(const struct device *dev, uint32_t *value)
{
	struct gpio_tca9539_drv_data *const drv_data =
		(struct gpio_tca9539_drv_data *const)dev->data;
	uint16_t buf;
...
static int gpio_tca9539_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value)
{
	struct gpio_tca9539_drv_data *const drv_data =
		(struct gpio_tca9539_drv_data *const)dev->data;
	uint16_t reg_out;
...
static int gpio_tca9539_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	return gpio_tca9539_port_set_masked_raw(dev, mask, mask);
}

static int gpio_tca9539_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	return gpio_tca9539_port_set_masked_raw(dev, mask, 0);
}

static int gpio_tca9539_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	struct gpio_tca9539_drv_data *const drv_data =
		(struct gpio_tca9539_drv_data *const)dev->data;
	uint16_t reg_out;
...
#ifdef CONFIG_GPIO_TCA9539_INTERRUPT
static void get_triggered_it(struct gpio_tca9539_drv_data *drv_data, uint16_t *trig_edge,
			     uint16_t *trig_level)
{
...
static void gpio_tca9539_interrupt_worker(struct k_work *work)
{
	struct gpio_tca9539_drv_data *const drv_data =
		CONTAINER_OF(work, struct gpio_tca9539_drv_data, interrupt_worker);
	const struct gpio_tca9539_config *const config = drv_data->instance->config;
...
static void gpio_tca9539_interrupt_callback(const struct device *dev, struct gpio_callback *cb,
					    gpio_port_pins_t pins)
{
	struct gpio_tca9539_drv_data *const drv_data =
		CONTAINER_OF(cb, struct gpio_tca9539_drv_data, gpio_callback);
...
static int gpio_tca9539_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
						enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	if ((mode != GPIO_INT_MODE_DISABLED) && (BIT(pin) & drv_data->reg_cache.dir) != BIT(pin)) {
		LOG_ERR("TCA9539[0x%X]: output pin cannot trigger interrupt", config->bus.addr);
		return -ENOTSUP;
	}
	...
	if (config->capabilities & PCA_HAS_INTERRUPT_MASK_REG) {
		...
		if (ret != 0) {
			LOG_ERR("TCA9539[0x%X]: failed to update int mask (%d)", config->bus.addr,
				ret);
			goto err;
		}
	}
	...
	if (active != drv_data->interrupt_active) {
		...
		if (ret != 0) {
			LOG_ERR("TCA9539[0x%X]: failed to configure interrupt "
				"on pin %d (%d)",
...
static int gpio_tca9539_manage_callback(const struct device *dev, struct gpio_callback *callback,
					bool set)
{
	const struct gpio_tca9539_config *const config = dev->config;
	struct gpio_tca9539_drv_data *const drv_data =
...
static DEVICE_API(gpio, gpio_tca9539_drv_api_funcs) = {
	.pin_configure = gpio_tca9539_config,
	.port_get_raw = gpio_tca9539_port_get_raw,
	.port_set_masked_raw = gpio_tca9539_port_set_masked_raw,
	.port_set_bits_raw = gpio_tca9539_port_set_bits_raw,
	.port_clear_bits_raw = gpio_tca9539_port_clear_bits_raw,
	.port_toggle_bits = gpio_tca9539_port_toggle_bits,
#ifdef CONFIG_GPIO_TCA9539_INTERRUPT
	.pin_interrupt_configure = gpio_tca9539_pin_interrupt_configure,
	.manage_callback = gpio_tca9539_manage_callback,
#endif
};
/**
 * @brief Initialization function of TCA9539
 *
 * @param dev Device struct
 * @return 0 if successful, failed otherwise.
 */
static int gpio_tca9539_init(const struct device *dev)
{
	const struct gpio_tca9539_config *const config = dev->config;
	struct gpio_tca9539_drv_data *const drv_data =
		(struct gpio_tca9539_drv_data *const)dev->data;
	...
	if ((config->capabilities & PCA_HAS_INTERRUPT) != 0) {
		...
		k_work_init(&drv_data->interrupt_worker, gpio_tca9539_interrupt_worker);

		/* Configure GPIO interrupt pin */
		if (!gpio_is_ready_dt(&config->int_gpio)) {
			LOG_ERR("TCA9539[0x%X]: interrupt GPIO not ready", config->bus.addr);
			return -ENODEV;
		}
		...
		gpio_init_callback(&drv_data->gpio_callback, gpio_tca9539_interrupt_callback,
				   BIT(config->int_gpio.pin));
		ret = gpio_add_callback(config->int_gpio.port, &drv_data->gpio_callback);
		if (ret != 0) {
			LOG_ERR("TCA9539[0x%X]: failed to add interrupt callback for"
				" pin %d (%d)",
				config->bus.addr, config->int_gpio.pin, ret);
			return ret;
		}
...
#define GPIO_TCA9539_DEVICE_INSTANCE(inst)                                                         \
	static const struct gpio_tca9539_config gpio_tca9539_##inst##_cfg = {                      \
		.common =                                                                          \
			{                                                                          \
				.port_pin_mask = GPIO_PORT_PIN_MASK_FROM_DT_INST(inst),            \
			},                                                                         \
		.bus = I2C_DT_SPEC_INST_GET(inst),                                                 \
		.capabilities = (DT_INST_PROP(inst, has_pud) ? PCA_HAS_PUD : 0) |                  \
				IF_ENABLED(CONFIG_GPIO_TCA9539_INTERRUPT,                          \
					   ((DT_INST_NODE_HAS_PROP(inst, interrupt_gpios)          \
						     ? PCA_HAS_INTERRUPT                           \
						     : 0) |                                        \
					    (DT_INST_PROP(inst, has_interrupt_mask_reg)            \
						     ? PCA_HAS_INTERRUPT_MASK_REG                  \
						     : 0) |)) 0,                                   \
		IF_ENABLED(CONFIG_GPIO_TCA9539_INTERRUPT,                                          \
			   (.int_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, interrupt_gpios, {}), ))};  \
                                                                                                   \
	static struct gpio_tca9539_drv_data gpio_tca9539_##inst##_drvdata = {                      \
		.reg_cache.input = 0x0,                                                            \
		...
		IF_ENABLED(CONFIG_GPIO_TCA9539_INTERRUPT, (.interrupt_active = false, ))};         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, gpio_tca9539_init, NULL, &gpio_tca9539_##inst##_drvdata,       \
			      &gpio_tca9539_##inst##_cfg, POST_KERNEL,                             \
			      CONFIG_GPIO_TCA9539_INIT_PRIORITY, &gpio_tca9539_drv_api_funcs);

DT_INST_FOREACH_STATUS_OKAY(GPIO_TCA9539_DEVICE_INSTANCE)
